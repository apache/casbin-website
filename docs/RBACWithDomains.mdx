...
For domain APIs (e.g. `GetAllUsersByDomain()`) with a **custom** domain token name or position, set the index after creating the enforcer. Full example: [rbac_with_domains_model.conf](https://github.com/apache/casbin/blob/master/examples/rbac_with_domains_model.conf).
