...
**Casbin K8s-Gatekeeper** is a Kubernetes admission webhook that uses Casbin for authorization. You define models and policies declaratively to allow or deny operations on any Kubernetes resource—no custom code in the webhook. Maintained by the Casbin community: [github.com/apache/casbin-k8s-gatekeeper](https://github.com/apache/casbin-k8s-gatekeeper).
