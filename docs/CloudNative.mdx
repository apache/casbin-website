---
id: cloud-native
title: Cloud Native
description: Cloud native authorization middlewares
keywords: [cloud native, authorization middleware, middlewares]
authors: [ErikQQY]
---

Casbin can run as an authorization layer in cloud-native stacks (Envoy, Istio, Traefik, KubeSphere, etc.). The projects below provide adapters or sidecars that call Casbin for allow/deny decisions.

## Projects by language

```mdx-code-block
<Tabs groupId="langs">
<TabItem value="Go" label="Go" default>
```

Project | Author | Description
---- | ---- | ----
[envoy-authz](https://github.com/apache/casbin-envoy-authz) | Casbin | Authorization middleware for [Istio](https://istio.io/) and [Envoy](https://envoyproxy.io/)
[kubesphere-authz](https://github.com/apache/casbin-kubesphere-authz) | Casbin | Authorization middleware for [kubeSphere](https://kubesphere.io/)
[casbin-forward-auth](https://github.com/grepplabs/casbin-forward-auth) | grepplabs | Authorization middleware for [Traefik](https://traefik.io/), [NGINX](https://nginx.org/), [HAProxy](https://www.haproxy.com/), [Envoy Gateway](https://gateway.envoyproxy.io/), [Istio](https://istio.io/) and [Envoy](https://www.envoyproxy.io/)

```mdx-code-block
</TabItem>
<TabItem value="Node.js" label="Node.js">
```

Project | Author | Description
---- | ---- | ----
[ODPF Shield](https://github.com/odpf/shield) | [Open Data Platform](https://github.com/odpf) | ODPF Shield is a cloud native role-based authorization-aware reverse-proxy service.

```mdx-code-block
</TabItem>
</Tabs>
```
